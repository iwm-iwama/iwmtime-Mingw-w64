//------------------------------------------------------------------------------
#define IWM_COPYRIGHT "(C)2021-2026 iwm-iwama"
...
INT main()
{
	...
	if (CreateProcessW(NULL, wpCmd, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
	{
		...
		LN(80);
		P(
			"\033[3G" CLR1 "Program" CLR2 "\033[12G"
			"%s"
			"\n"
			"\033[3G" CLR1 "Execute" CLR2 "\033[12G"
			"%.3f sec"
			"\n"
			"\033[3G" CLR1 "Memory" CLR2 "\033[12G"
			"%.3f MB"
			"\n",
			mpCmd, ((iFinfo_ftimeToINT64(exitTime) - iFinfo_ftimeToINT64(creationTime)) / NANO100), (pmc.PeakPagefileUsage / MB));
		P1(CLR1);
		LN(80);
		P2(IESC_RESET);
		ifree(mpCmd);
